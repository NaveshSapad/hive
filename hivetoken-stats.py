...
def get_chart(df_user_details,token,sym_list,sym):
    ...
    if(token):
        if sym!='All':  # Then a particular symbol is selected in the selectbox .
            ...
            st.write('<div class="card"><div class="card-header"><center>Total '+sym+' from Jan 1 to Feb 9 : '+ '%.6f' % sum_sym+' '+sym+' , In HIVE = '+'%.6f' % total +'.</center>',unsafe_allow_html=True)
